...
| Testing | B | Unit tests, Postgres integration tests, build checks, upstream ARD conformance runs, GitHub Actions CI, and `make test-e2e` cover core ingestion/search/admin/artifact-onboarding paths. | Consider an optional scheduled E2E workflow once external artifact availability and rate limits are better understood. |
...
| Security | C | Validation enforces `urn:air:`, value/reference exclusivity, URLs, and basic catalog shape. Admin API routes are disabled by default and guarded by a bearer token when enabled. RBAC, audit logs, signatures, and trust manifest verification are still pending. | Add role-based authorization, audit logging, and trust verification design before broader network exposure. |
