...
| Testing | B | Unit tests, Postgres integration tests, build checks, upstream ARD conformance runs, GitHub Actions CI, `make test-e2e`, and `make test-compose` cover core ingestion/search/admin/artifact-onboarding, pagination, and deployment paths, including live MCP, Skill, and OpenAPI artifacts plus a checked-in A2A fixture. | Consider an optional scheduled E2E workflow once external artifact availability and rate limits are better understood. |
