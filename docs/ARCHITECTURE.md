...
- Request correlation: Gin middleware preserves or generates `X-Request-ID`, returns it
  ...
  fetches, source digest verification, and attestation digest verification.
- Trace context: Gin middleware accepts or generates W3C `traceparent`, returns the
  ...
  digest, attestation digest, and admin client requests.
...
- Verification engine: schema-level checks cover `urn:air:`, required fields,
  ...
  verification can require every attestation to carry a pinned digest. Detached compact
  JWS `trustManifest.signature` values can be verified against explicit Ed25519 trust
  anchors supplied in either ard's native format or local JWKS OKP/Ed25519 format, and
  strict verification can require every catalog entry to carry a verifiable signature.
...
- Treat detached compact JWS verification as explicit operator trust-anchor verification.
  ...
  generated by `ard` with `signature` removed; it is not automatic DID, SPIFFE,
  certificate, remote JWKS, or key-discovery verification.
