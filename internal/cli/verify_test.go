...
func TestVerifyCatalogDiscoverSPIFFERequiresSPIFFEIdentity(t *testing.T) {
	tempDir := t.TempDir()
	catalogPath := filepath.Join(tempDir, "ai-catalog.json")
	if err := os.WriteFile(catalogPath, []byte(`{
  "specVersion": "1.0",
  "entries": [
	...
	command.SetOut(&output)
	command.SetErr(&output)
	command.SetArgs([]string{"verify", "catalog", catalogPath, "--jws-discover-spiffe"})
	err := command.Execute()
	if err == nil || !strings.Contains(err.Error(), "--jws-discover-spiffe") {
		t.Fatalf("expected SPIFFE discovery anchor error, got %v output %s", err, output.String())
	}
}
